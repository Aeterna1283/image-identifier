...
#read an image test
img = cv2.imread('newimagesize-248.webp')
plt.imshow(img)
...
while True:
    ...
    ClassIndex, confid, bbox = model.detect(frame, confThreshold=0.55)

    if ClassIndex is not None and len(ClassIndex) != 0:
        for ClassInd, conf, boxes in zip(ClassIndex.flatten(), confid.flatten(), bbox):
            if ClassInd <= 80:
